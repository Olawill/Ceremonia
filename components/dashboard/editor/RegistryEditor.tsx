        ...
        <>
          ...
          {scrapedItems.length > 0 && (
            <div
              ...
              style={{ background: "#D4AF3705", border: "1px solid #D4AF3730" }}
            >
              ...
              <button
                ...
                disabled={
                  addingAll || !scrapedItems.some((i) => i.selected && !i.error)
                }
              ...
              >
                {addingAll ? (
                  <>
                    <Loader2Icon className="size-3.5 animate-spin" /> Adding…
                  </>
                ) : (
                  <>
                    <CheckIcon className="size-3.5" /> Add{" "}
                    {scrapedItems.filter((i) => i.selected && !i.error).length}{" "}
                    items to Registry
                  </>
                )}
              </button>
          ...
          {addMode === "manual" && (
            ...
            >
                ...
                <div className="flex gap-2 flex-wrap mb-1.5">
                  {RETAILER_SUGGESTIONS.map((r) => (
                    <button
                      key={r}
                      type="button"
                      onClick={() => setNewItem((p) => ({ ...p, retailer: r }))}
              ...
              <Field label="Product Link" hint="Paste the Amazon/retailer URL">
                <Input
                  value={newItem.productUrl}
                  onChange={(e) =>
                    setNewItem((p) => ({ ...p, productUrl: e.target.value }))
                  }
                  placeholder="https://amazon.co.uk/…"
                />
              </Field>
