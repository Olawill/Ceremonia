...
**Live demo:** [ceremonia.app/wedding/demo](https://ceremonia.app/wedding/demo)
...
→ Rewrite to: /wedding/emma-birthday

Request: my-wedding.com  (custom domain)
→ Not a ceremonia.app subdomain
→ Rewrite to: /wedding/domain/my-wedding.com
...
In local development, `demo.localhost:3000` maps to `/wedding/demo`.
...
**`PreviewFrame.tsx`** — the right panel renders an `<iframe>` pointing to `/wedding/preview`. It listens for `CURTAIN_OPEN` and `DATE_REVEALED` postMessages to show the jump-to-section nav bar. On mobile, the preview opens as a slide-over sheet.
...
- `app/wedding/[slug]/page.tsx` — `<title>`, OpenGraph, description
- `app/(dashboard)/app/editor/[slug]/page.tsx` — editor page title
- `app/wedding/domain/[host]/page.tsx` — custom domain page title
...
```
GET    /api/weddings              → list user's events
POST   /api/weddings              → create event
GET    /api/weddings/:slug        → get event (owner only)
PATCH  /api/weddings/:slug        → update event (plan-gated fields)
DELETE /api/weddings/:slug        → delete event
...
**E2E tests** (`tests/e2e/`) use Playwright against a running dev/preview server. The `/wedding/demo` route is the primary anchor — it renders `DEMO_WEDDING_CONFIG` without auth or DB, making it a stable test target. Auth-gated dashboard tests require `tests/e2e/.auth/user.json` (generated by `auth.setup.ts`).
...
Visit `http://localhost:3000` for the marketing page and `http://localhost:3000/app/dashboard` for the editor. The demo invitation is at `http://localhost:3000/wedding/demo`.
