...
// /** Create a tiny real File but override .size via Object.defineProperty so we
//  *  can test size-limit logic without allocating megabytes of memory. */
...
// // ── POST /api/upload — via app.handle with real multipart ────────────────────
// // Using a real FormData with tiny files (4 bytes) so the multipart parser
// // completes immediately. Timeout raised to 15s as a safety net.
...
//   overrideSize?: number,
// ): Promise<{ status: number; body: unknown }> {
//   // For size-limit tests, create a buffer of exactly that size.
//   // For normal tests, use 4 bytes to keep things fast.
//   const byteLength = overrideSize ?? 4;
...
//   const res = await app.handle(
//     new Request("http://localhost/upload", {  // <-- note: /upload not /api/upload
//       method: "POST",
...
//     const r = await uploadReq("photo.jpg", "image/jpeg", "photo");
//     expect(r.status).toBe(401);
//   });

//   it(
//     "returns 403 when free plan tries to upload audio (posthog captured)",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-1");
//       mockDb.select.mockReturnValueOnce(planSelect("free"));
//       const r = await uploadReq("music.mp3", "audio/mpeg", "audio");
...
//   it(
//     "returns 400 when photo slot receives a non-image file",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-1");
//       mockDb.select.mockReturnValueOnce(planSelect("pro"));
//       const r = await uploadReq("music.mp3", "audio/mpeg", "photo");
...
//   it(
//     "returns 400 when audio slot receives a non-audio file",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-1");
//       mockDb.select.mockReturnValueOnce(planSelect("starter"));
//       const r = await uploadReq("photo.jpg", "image/jpeg", "audio");
//       expect(r.status).toBe(400);
//       expect((r.body as { message: string }).message).toMatch(
//         /must be an audio/,
//       );
...
//   it("returns 400 when photo exceeds 10MB", { timeout: 15000 }, async () => {
//     authed.mockResolvedValue("user-1");
//     mockDb.select.mockReturnValueOnce(planSelect("pro"));
//     const r = await uploadReq(
//       "big.jpg",
//       "image/jpeg",
//       "photo",
//       10 * 1024 * 1024 + 1,
...
//   it("returns 400 when audio exceeds 20MB", { timeout: 15000 }, async () => {
//     authed.mockResolvedValue("user-1");
//     mockDb.select.mockReturnValueOnce(planSelect("starter"));
//     const r = await uploadReq(
//       "big.mp3",
...
//   it(
//     "returns 200 with blob URL for valid photo (free plan)",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-1");
//       mockDb.select.mockReturnValueOnce(planSelect("free"));
//       const r = await uploadReq("hero.jpg", "image/jpeg", "photo");
...
//   it(
//     "returns 200 with blob URL for valid audio (starter plan)",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-1");
//       mockDb.select.mockReturnValueOnce(planSelect("starter"));
//       mockPut.mockResolvedValueOnce(AUDIO_BLOB);
//       const r = await uploadReq("song.mp3", "audio/mpeg", "audio");
//       expect(r.status).toBe(200);
...
//   it("photo exactly at 10MB is allowed", { timeout: 15000 }, async () => {
//     authed.mockResolvedValue("user-1");
//     mockDb.select.mockReturnValueOnce(planSelect("free"));
//     const r = await uploadReq(
//       "ok.jpg",
...
//   it("audio exactly at 20MB is allowed", { timeout: 15000 }, async () => {
//     authed.mockResolvedValue("user-1");
//     mockDb.select.mockReturnValueOnce(planSelect("starter"));
//     mockPut.mockResolvedValueOnce(AUDIO_BLOB);
//     const r = await uploadReq(
...
//   it(
//     "calls put with correct photo path pattern",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-abc");
//       mockDb.select.mockReturnValueOnce(planSelect("pro"));
//       await uploadReq("wedding.png", "image/png", "photo");
...
//   it(
//     "calls put with correct audio path pattern",
//     { timeout: 15000 },
//     async () => {
//       authed.mockResolvedValue("user-xyz");
//       mockDb.select.mockReturnValueOnce(planSelect("agency"));
//       mockPut.mockResolvedValueOnce(AUDIO_BLOB);
//       await uploadReq("track.mp3", "audio/mpeg", "audio");
//       const [path] = mockPut.mock.calls[0] as [string, ...unknown[]];
//       expect(path).toMatch(/^audios\/user-xyz\/\d+\.mp3$/);
//     },
//   );

//   it("pro plan can upload audio", { timeout: 15000 }, async () => {
//     authed.mockResolvedValue("user-1");
//     mockDb.select.mockReturnValueOnce(planSelect("pro"));
//     mockPut.mockResolvedValueOnce(AUDIO_BLOB);
//     const r = await uploadReq("track.wav", "audio/wav", "audio");
//     expect(r.status).toBe(200);
//   });
// });
...
//     authed.mockResolvedValue("user-1");
//     mockDb.select.mockReturnValueOnce(planSelect("free"));
//     // The Elysia body validator enforces pattern: "^https?://" — send a raw invalid value
//     // by bypassing the schema check with a valid-looking but unparseable URL
