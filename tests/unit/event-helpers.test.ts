...
describe("getHost1Name", () => {
  it("returns bride for wedding config", () => {
    expect(getHost1Name(DEMO_WEDDING_CONFIG)).toBe(DEMO_WEDDING_CONFIG.bride);
  });

  it("returns host1Name when set", () => {
    const config = { ...DEMO_WEDDING_CONFIG, host1Name: "Sarah" };
...
describe("getHost2Name", () => {
  it("returns groom for wedding config", () => {
    expect(getHost2Name(DEMO_WEDDING_CONFIG)).toBe(DEMO_WEDDING_CONFIG.groom);
  });

  it("returns undefined for single-host event (birthday)", () => {
    const config = { ...DEMO_WEDDING_CONFIG, eventType: "birthday" as const };
...
describe("getHostsLabel", () => {
  it("returns 'Bride & Groom' format for wedding", () => {
    const label = getHostsLabel(DEMO_WEDDING_CONFIG);
